...
/*   Updated: 2024/09/24 14:45:33 by plashkar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 *@brief The flood fill algorithm to check if the map has invalid borders.
 *
...
int	flood_fill(t_map *map, int **visited, int x, int y)
{
	...
	{
		if ((is_in_bounds(x + 1, y, map) && map->map_array[x + 1][y] == ' ') ||
			(is_in_bounds(x - 1, y, map) && map->map_array[x - 1][y] == ' ') ||
			(is_in_bounds(x, y + 1, map) && map->map_array[x][y + 1] == ' ') ||
			(is_in_bounds(x, y - 1, map) && map->map_array[x][y - 1] == ' '))
			return (0);
	}
